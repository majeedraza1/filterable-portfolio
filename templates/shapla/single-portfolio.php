...
$rp_grid 			= sprintf('grid %1$s %2$s %3$s %4$s', esc_attr( $options['columns_phone'] ), esc_attr( $options['columns_tablet'] ), esc_attr( $options['columns_desktop'] ), esc_attr( $options['columns'] ));
$term_ids 			= array_map( function( $tag ) { return $tag->term_id; }, $terms );
$skill_ids 			= array_map( function( $tag ) { return $tag->term_id; }, $skills );
$args = array(
	'post_type'      => 'portfolio',
